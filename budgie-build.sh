...
make_packages() {
    setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "$(grep -h -v ^# ${script_path}/budgie-packages.{both,${arch}})" install
}
...
make_customize_airootfs() {
    ...
    setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r '/root/budgie_customize_airootfs.sh' run
    rm ${work_dir}/${arch}/airootfs/root/budgie_customize_airootfs.sh
}
