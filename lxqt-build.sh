#!/bin/bash
# ARCHISO BUILD COMMAND: "sudo ./lxqt-build.sh -v -o out"
...
make_packages() {
    setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -p "$(grep -h -v ^# ${script_path}/lxqt-packages.{both,${arch}})" install
}
...
make_customize_airootfs() {
    ...
    setarch ${arch} mkarchiso ${verbose} -w "${work_dir}/${arch}" -C "${work_dir}/pacman.conf" -D "${install_dir}" -r '/root/lxqt_customize_airootfs.sh' run
    rm ${work_dir}/${arch}/airootfs/root/lxqt_customize_airootfs.sh
}
...
# Build ISO
make_iso() {
    out_filename="archman-$(date +%y%m)_x86_64.iso"
